        ...
        Try
            ...
            If isAfter Then
                Dim endDate_after = (From p In Context.AT_PERIOD
                              ...
                              Select p.START_DATE).FirstOrDefault
                If endDate_after IsNot Nothing Then
                    ...
                    Dim period_after = (From p In Context.AT_PERIOD
                                        Where p.END_DATE = endDate_after).FirstOrDefault

                    If period_after IsNot Nothing Then
                        period_id = period_after.ID
                        Dim countOrg = (From p In Context.AT_PERIOD
                        From po In Context.AT_ORG_PERIOD.Where(Function(f) f.PERIOD_ID = p.ID)
                        From k In Context.SE_CHOSEN_ORG.Where(Function(f) po.ORG_ID = f.ORG_ID And
                                                                  f.USERNAME.ToUpper = log.Username.ToUpper)
                        Where p.ID = period_id And
                        po.ORG_ID <> 46).Count
                        If countOrg = 0 Then
                            Return True
                        End If
...
#Region "Máy chấm công"
    ...
    Public Function GetTerminalMeal(ByVal obj As AT_TERMINALSDTO, ByVal log As UserLog) As DataTable
        Try
            Using cls As New DataAccess.QueryData
                Dim dtData As DataTable = cls.ExecuteStore("PKG_ATTENDANCE_LIST.GET_TERMINAL_MEAL",
                                               New With {.P_ID = obj.ID,
                                                         .P_CUR = cls.OUT_CURSOR})
                Return dtData
            End Using
    ...
    Public Function UpdateTerminalLastTime(ByVal obj As AT_TERMINALSDTO,
        ...
        Try
            ...
            Else
                Using cls As New DataAccess.NonQueryData
                    cls.ExecuteStore("PKG_ATTENDANCE_LIST.GET_UPDATE_MEAL_LASTIME",
                                     New With {.P_ID = obj.ID,
                                               .P_DATE = obj.MODIFIED_DATE})
                End Using
            End If
    ...
    Public Function UpdateTerminalStatus(ByVal obj As AT_TERMINALSDTO,
                                        Optional ByVal isMeal As Boolean = False) As Boolean
        Try
            If Not isMeal Then
                Using cls As New DataAccess.NonQueryData
            ...
            Else
                Using cls As New DataAccess.NonQueryData
                    cls.ExecuteStore("PKG_ATTENDANCE_LIST.GET_STATUS_MEAL_LASTIME",
                                     New With {.P_ID = obj.ID,
                                               .P_DATE = obj.MODIFIED_DATE,
                                               .P_STATUS = obj.TERMINAL_STATUS,
                                               .P_ROW = obj.TERMINAL_ROW})
