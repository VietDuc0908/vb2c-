    ...
    Public Function GetComboboxData(ByRef cbxData As ComboBoxDataDTO) As Boolean
        Try
            ...
            If cbxData.GET_LIST_SHIFT_SUNDAY Then
                cbxData.LIST_LIST_SHIFT_SUNDAY = (From p In Context.AT_TIME_MANUAL Where p.ACTFLG = "A" And p.CODE = "OFF" Order By p.NAME Descending
                                               Select New AT_TIME_MANUALDTO With {
                                                   .ID = p.ID,
                                                   .CODE = p.CODE,
                                                   .NAME_VN = "[" & p.CODE & "] " & p.NAME}).ToList
            ...
            If cbxData.GET_LIST_TERMINALS_MEAL Then
                cbxData.LIST_LIST_TERMINALS_MEAL = (From p In Context.AT_TERMINALS_MEAL
                                            Where p.ACTFLG = "A"
                                            Order By p.TERMINAL_NAME
                                            Select New AT_TERMINALS_MEALDTO With {
                                                .ID = p.ID,
                                                .TERMINAL_NAME = p.TERMINAL_NAME}).ToList
            End If

            If cbxData.GET_LIST_TIME_TYPE Then
                cbxData.LIST_LIST_TIME_TYPE = (From p In Context.OT_OTHER_LIST Join t In Context.OT_OTHER_LIST_TYPE On p.TYPE_ID Equals t.ID
                                             Where p.ACTFLG = "A" And t.CODE = "TIME_TYPE" Order By p.CREATED_DATE Descending
                         Select New OT_OTHERLIST_DTO With {
                             .ID = p.ID,
                             .CODE = p.CODE,
                             .NAME_EN = p.NAME_EN,
            ...
            'loại suất ăn
            If cbxData.GET_LIST_RATION Then
                cbxData.LIST_LIST_RATION = (From p In Context.OT_OTHER_LIST Join t In Context.OT_OTHER_LIST_TYPE On p.TYPE_ID Equals t.ID
                                             Where p.ACTFLG = "A" And t.CODE = "RATION" Order By p.CREATED_DATE Descending
                         Select New OT_OTHERLIST_DTO With {
                             .ID = p.ID,
                             .CODE = p.CODE,
                             .NAME_EN = p.NAME_EN,
            ...
            'Danh sách bếp
            If cbxData.GET_LIST_KITCHEN Then
                cbxData.LIST_LIST_KITCHEN = (From p In Context.AT_KITCHEN Order By p.CREATED_DATE Descending
                         Select New OT_OTHERLIST_DTO With {
                             .ID = p.ID,
                             .CODE = p.KITCHEN_CODE,
            ...
            If cbxData.GET_LIST_MEAL Then
                cbxData.LIST_LIST_MEAL = (From p In Context.AT_MEAL Order By p.ID
                         Select New OT_OTHERLIST_DTO With {
                             .ID = p.ID,
                             .CODE = p.CODE,
                             .NAME_VN = p.NAME
            ...
            If cbxData.GET_LIST_PARTNER Then
                cbxData.LIST_LIST_PARTNER = (From p In Context.OT_OTHER_LIST Join t In Context.OT_OTHER_LIST_TYPE On p.TYPE_ID Equals t.ID
                                             Where p.ACTFLG = "A" And t.CODE = "PARTNER_TYPE" Order By p.CREATED_DATE Descending
                         Select New OT_OTHERLIST_DTO With {
                             .ID = p.ID,
                             .CODE = p.CODE,
                             .NAME_VN = p.NAME_VN
